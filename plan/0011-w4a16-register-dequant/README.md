...
## v3 record (2026-09-17): the paired A/B ran, verdict SPLIT

The bounded fix unlocked BN128/S2: the permute is per-64-block, so
the absolute stored half is 64U + 8c + 2w' (v2 dropped the 64U base
and used w for w&3). Results:
...
Marlin at M>=64) is engine-lane work, its own task; the large-M gap
